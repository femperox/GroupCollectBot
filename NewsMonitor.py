...
def monitorNews(rcpns, theme, origin):
    items = []
    while True:
        try:
            items = YouLoveItApi.getNews()
            logger_news.info(f"[SEEN-{theme}] len {len(items)} news")
            items = getNotSeenNews(news_ids = items, origin = origin)
            if items:
                for item in items:
                ...
                insertNewSeenNews(news_ids = items, origin = origin)
...
if __name__ == "__main__":
    ...
    for conf in conf_list:
        threads.append(threading.Thread(target=monitorNews, args=(conf["rcpns"], conf["theme"], NewsConsts.NewsOrigin.youloveit)))
